          ...
          <div class="func-title">
            ...
            <el-popover placement="bottom-start" width="400" trigger="hover">
              ...
              <ul class="history-list">
                <li
                  class="history-msg"
                  v-for="(msg, idx) in msgHistory"
                  :key="msg + idx"
                  @click="setHistoryMsg(msg)"
                  :title="msg"
                >
                  {{ msg }}
                  <el-button type="primary" plain size="mini">resend</el-button>
                </li>
              </ul>
              <i
                title="发送历史"
                slot="reference"
                class="el-icon-time history-trigger"
              ></i>
            </el-popover>
          </div>
          <el-input
            v-model="content"
            placeholder="请输入内容"
            @keyup.native.ctrl.enter="manualSend"
            type="textarea"
            clearable
            :rows="10"
          >
...
export default {
  ...
  methods: {
    ...
    autoHeartBeat: function() {
      ...
      if (_this.autoSend === true) {
        ...
        _this.autoTimer = setInterval(function() {
          ...
          _this.sendData(_this.heartBeatContent, true);
        }, _this.heartBeatSecond * 1000);
      }
    },
    setHistoryMsg(history) {
      this.content = history;
      this.sendData(history, true);
    },
    ...
    sendData: function(raw, autoSend) {
      var _this = this;
      var data = raw;
